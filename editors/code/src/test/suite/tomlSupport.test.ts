...
`.trim();
    ...
    // ── [codegen.openapi] (parity with Rust validate_codegen) ──────────────

    const withCodegen = (body: string): string => `
[package]
name = "my-app"
...
`.trim();
    ...
    test('openapi codegen missing required keys is flagged', () => {
        const text = withCodegen('version = "20.0.0"');
        const diags = validateManifest(text);
    ...
    test('openapi codegen version below 18 is flagged', () => {
        const text = withCodegen('version = "17.0.0"\nspec = "specs/api.yaml"\nbase_package = "com.example.api"');
        assert.ok(hasError(validateManifest(text), '18.0.0 or newer'));
    });

    test('openapi codegen non-numeric version is flagged', () => {
        const text = withCodegen('version = "latest"\nspec = "specs/api.yaml"\nbase_package = "com.example.api"');
        assert.ok(hasError(validateManifest(text), 'valid Fabrikt version'));
    });

    test('openapi codegen absolute spec path is flagged', () => {
        const text = withCodegen('version = "20.0.0"\nspec = "/tmp/api.yaml"\nbase_package = "com.example.api"');
        assert.ok(hasError(validateManifest(text), 'relative path inside the project'));
    });

    test('openapi codegen parent-traversal spec path is flagged', () => {
        const text = withCodegen('version = "20.0.0"\nspec = "../outside/api.yaml"\nbase_package = "com.example.api"');
        assert.ok(hasError(validateManifest(text), '".."'));
    });

    test('openapi codegen unsupported spec extension is flagged', () => {
        const text = withCodegen('version = "20.0.0"\nspec = "specs/api.txt"\nbase_package = "com.example.api"');
        assert.ok(hasError(validateManifest(text), '.yaml'));
    });

    test('openapi codegen invalid base_package is flagged', () => {
        const text = withCodegen('version = "20.0.0"\nspec = "specs/api.yaml"\nbase_package = "com..example"');
        assert.ok(hasError(validateManifest(text), 'dot-separated identifiers'));
    });
