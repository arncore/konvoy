...
suite('TomlCompletionProvider', () => {
    ...
    suiteSetup(() => {
        ...
        disposables = registerTomlSupport({} as vscode.ExtensionContext);
    });
...
suite('TomlHoverProvider', () => {
    // The providers were already registered in TomlCompletionProvider suiteSetup.
    // Re-register to keep this suite independent (VS Code deduplicates internally).
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const { registerTomlSupport } = require('../../tomlSupport');
    let disposables: vscode.Disposable[] = [];
